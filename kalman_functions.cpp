    ...
    while(std::getline(infile,line)){
    	...
    	if(!line.empty()){
    		switch(count_param_positions){
    		    case 0: result.working_dir=line.c_str();
    		            break;
    		    case 1: result.data_dir=line.c_str();
    		        	break;
    		    case 2: result.positions_angles_data=line.c_str();
    		        	 break;
    		    case 3: result.length_width_data=line.c_str();
    		        	 break;
    		    case 4: result.pictures_filename_tpl=line.c_str();
    		        	break;

    		    case 5: result.subregion_x=atoi(line.c_str());
    		            break;
    		    case 6: result.subregion_y=atoi (line.c_str());
    		            break;
    		    case 7: result.dx=atoi (line.c_str());
    		        	break;
    		    case 8: result.dy=atoi (line.c_str());
    		        	break;

    		    case 9: result.time_start=atoi(line.c_str());
    		        	break;
    		    case 10: result.time_stop=atoi(line.c_str());
    		        	break;
    		    case 11: result.time_step=atoi(line.c_str());
    		        	break;

    		    case 12: result.width_min=atof(line.c_str());
    		             break;

    		    case 13: result.v_init=atof(line.c_str());
    		        	 break;
    		    case 14: result.initial_noise_x=atof(line.c_str());
    		        	 break;
    		    case 15: result.initial_noise_y=atof(line.c_str());
    		        	 break;
    		    case 16: result.initial_noise_vx=atof(line.c_str());
    		        	 break;
    		    case 17: result.initial_noise_vy=atof(line.c_str());
    		        	 break;

    		    case 18: result.observe_velocity=string2bool(line.c_str());
                         break;

    		    case 19: result.weight_q_pos=atof(line.c_str());
    		        	 break;
    		    case 20: result.weight_q_vel=atof(line.c_str());
    		        	 break;
    		    case 21: result.weight_r_pos=atof(line.c_str());
    		        	 break;
    		    case 22: result.weight_r_vel=atof(line.c_str());
    		        	 break;

    		    case 23: result.edge_region=atof(line.c_str());
    		        	 break;

    		    case 24: result.angfactor=atof(line.c_str());
    		        	 break;
    		    case 25: result.rating_threshold=atof(line.c_str());
    		        	 break;

    		    case 26: result.multiple_pred_threshold=atoi(line.c_str());
    		             break;

    		    case 27: result.print_index=string2bool(line.c_str());
    		        	break;
    		    case 28: result.plot_cov=string2bool(line.c_str());
    		        	break;

    		    case 29: result.input_kind=atoi(line.c_str());
    		             break;
...
void read_data_len(std::string filename, size_dict_t &sizes, int time_start, int time_stop, int time_step)
{
    ...
    std::vector<Vector> sizes_v; // vector containing the lenght and the width of all rectangles for one frame

    std::getline(in, firstline); // skip the first line, use it as header
...
void add_new_bacteria(int t, list<int> pos_indices, pos_dict_t &positions, ang_dict_t &angles, matrix_dict_t &state_updated_dict,
		matrix_dict_t &updated_p_dict, int &highest_index, vector<string> filenames, configuration_parameters_t params, set<int> &new_bac){
...
double rate_match(Vector pos1, Vector pos2, double ang1, double ang2, double p_x, double p_y, double pospara_factor, double posorth_factor, double ang_factor, double dx)
{
    // vertical and parallel distance
	std::tuple<double, double> com_distance;
    com_distance = ParaOrthDistance(pos1, pos2, ang1, ang2);

    double pospara_rating = pospara_factor * std::get<0>(com_distance) / dx;
    double posorth_rating = posorth_factor * std::get<1>(com_distance) / dx;
    ...
    double ang_rating = ang_factor * AngleDifference90(ang1, ang2) / (PI/2);
...
// function for matrix inversion using LU factorization (inspired from https://savingyoutime.wordpress.com/2009/09/21/c-matrix-inversion-boostublas/)
