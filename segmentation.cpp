 ...
 * @author all this code is from tracking.cpp https://github.com/SwarmingSoft/ImageProcessing
 */
...
//    }
	...
	params = read_parameters(std::string(argv[1]) + "/config.txt");


	std::string INPATH = params.working_dir + params.data_dir; // path with trailing slash
	std::string FILENAMETPL = params.pictures_filename_tpl; // "img_%09d_00-BF_EGFP_000.tif"
	int START = params.time_start; // 0
	int STOP = params.time_stop; // 1000
	int X = params.subregion_x; //512
	int Y = params.subregion_y; //325
	int DX = params.dx; //512
	int DY = params.dy; //512
	std::string OUTPATH = params.working_dir + params.data_dir; // path with trailing slash
...
/*for (real anglee = 0.; anglee < 360.; anglee+=1.)
{
	...
	int width = params.seg_width;//2; //3; //5; //of test rectangle >=1
	int height = params.seg_length; //40; //80; //of test rectangle =>1
	real delta_angle = params.seg_delta_angle; //1.; //20.; //1.; //rotation of rectangle >0, <180 // in degree
	real ignore_ratio = params.seg_ignore_ratio; //0.00; //0.015; //maximum area error in binary picture <=1, >=0
	int delta_x = params.seg_delta_x; //2; //1; //translation of rectangle in x >=1
	int delta_y = params.seg_delta_y; //2; //1; //translation of rectangle in y >=1
	//parameters for rectangle grouping in contours
	real max_allowed_angle_diff = params.seg_max_allowed_angle_diff; //2.; //18.; //in degree
	real max_allowed_center_diff_factor = params.seg_max_allowed_center_diff_factor; //0.6; // 1.2; //factor of "height"
	real max_allowed_perp_center_diff_factor = params.seg_max_allowed_perp_center_diff_factor;//0.4; // 8.; //factor of "width"
	//parameters for filtering out additional overlapping rects in some contour
	real max_area_overlapp = params.seg_max_area_overlapp;//0.52; // 0.7; //factor of area of smaller of 2 compared rects
